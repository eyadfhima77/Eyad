...
def subc():
   ...
   id="|".join(uuid)
   ses=requests.get("https://pastebin.com/nY63373S").text
   if id in ses:
   	pass
   else:
   	print("Your Id "+id)
...
def checks(oks,cps,twf):
    if not len(oks) != 0:
        pass
    if len(cps) != 0:
        print('\n\n\x1b[1;97m TOTAL  OK : \x1b[1;97m %s  \x1b[1;97mEyad-OK.txt' % (     
             H, P, str(len(oks))))
        print('\x1b[1;97m TOTAL CP :\x1b[1;97m   %s \x1b[1;97mEyad-CP.txt' %
              (H, P, str(len(cps))))
        print('\x1b[1;97m TOTAL 2F :\x1b[1;97m   %s \x1b[1;97mEyad-2F.txt' %
              (H, P, str(len(twf))))
